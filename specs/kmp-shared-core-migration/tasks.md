...
- 2026-07-19：V2 兼容实现完成：Core 增加 `legacy_v2_version`（V2 `ts/ctr/dev` LWW 元数据）及 `LegacyV2SyncFacade`，从 KMP task/habit/category 投影并回放三份 V2 快照；iOS `KMPV2WebDAVSyncService` 仅负责三份文件的 WebDAV ETag 读写，设置页默认选择 V2、可切换原生 changelog。更新后的 XCFramework 已替换到 Vendor；主 App 编译未发现 Swift 错误，仍被既有 `actool` 空资源异常终止。
